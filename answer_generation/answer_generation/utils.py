"""Prompt templates, result types, and metrics for answer generation and judging."""
...
# ---------------------------------------------------------------------------
# Prompts (ViDoRe v3 paper: Figure 25 for generation, Figure 24 for judging)
# ---------------------------------------------------------------------------
...
You are an expert at answering queries based on document images.
The images above show the relevant document pages.

Based on the document pages shown, answer the following query: {query}
...
You are an expert at answering queries based on documents.
The images above show relevant document pages.
Here are also the extracted texts from relevant documents:

{documents}

Based on the document pages and texts above, answer the following query: {query}
...
def format_documents(texts: list[str]) -> str:
    """Format a list of markdown texts as a numbered document list."""
...
def parse_judge_response(query_id: int, raw: str) -> JudgmentResult:
    ...
    # Regex fallback: find the judgment value anywhere in the raw response
