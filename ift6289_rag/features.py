...
from ift6289_rag.config import PROCESSED_DATA_DIR
...
def precompute_image_embeddings(
    model,
    ds_corpus,
    save_dir: str = "colembed_cache_pages_fp16",
    batch_size: int = 32,
    num_workers: int = 8,
):
...
def load_precomputed_image_embeddings(
    ds_corpus: Dataset, save_dir: str = "colembed_cache_pages_fp16"
):
    ...
    pages_embeddings = [
        torch.load(save_dir / f"{cid}.pt", map_location="cpu")["emb"].to(
            torch.bfloat16
        )
...
def precompute_query_embeddings(
    model,
    ds_queries,
    save_dir: str = "colembed_cache_queries_fp16",
):
    save_dir = PROCESSED_DATA_DIR / save_dir
    os.makedirs(save_dir, exist_ok=True)
